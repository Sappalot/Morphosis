...
public class GlobalPanel : MonoSingleton<GlobalPanel> {
	...
	private void Update() {
		...
		if (updateTimeCount > updatePeriod) {
			...
			if (debugPopup.activeInHierarchy) {
				fps.text = string.Format("FPS: {0:F1}", frameRate);
				pps.text = CreatureEditModePanel.instance.mode == PhenoGenoEnum.Phenotype ? string.Format("PPS: {0:F1}", physicsUpdatesPerSecond) : "PPS: ---";
