		...
		if (isDirty) {
			...
			if (GetMode() == PhenoGenoEnum.Phenotype) {
				if (CellPanel.instance.selectedCell != null) {
					footerPanel.SetProductionEffectText(selectedCell.effectProductionInternalUp, GlobalSettings.instance.phenotype.leafCellEffectCost);
				...
				footerPanel.SetProductionEffectText(string.Format("Production Effect: [exposure (0...1)] * [creature size factor ({0:F2}...{1:F2})] * {2:F2} - {3:F2} W", bodySize1Value, bodySize100Value, GlobalSettings.instance.phenotype.leafCellSunMaxEffect, GlobalSettings.instance.phenotype.leafCellEffectCost));
